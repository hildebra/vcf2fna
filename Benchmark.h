...
public:
    ...
    void  now_total_time(ostream& os = std::cout) {
        auto now_time = high_resolution_clock::now();
        auto duration = duration_cast<microseconds>(now_time - last_time);
        float time_loc = (float)duration.count();
        uint hours = time_loc / 3600000000;
        time_loc -= hours * 3600000000;
        uint mins = time_loc / 60000000;
        time_loc -= mins * 60000000;
        uint secs = time_loc / 1000000;
        time_loc -= secs * 1000000;
        uint msecs = time_loc / 1000;
